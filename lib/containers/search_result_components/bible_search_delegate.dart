...
/// [BibleSearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [BibleSearchDelegate] can only be associated with one active [showBibleSearch]
/// call. Call [BibleSearchDelegate.close] before re-using the same delegate instance
/// for another [showBibleSearch] call.
///
...
/// The animation provided by [BibleSearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [BibleSearchDelegate] to define the content of the search page.
Future<T> showBibleSearch<T>({
  @required BuildContext context,
  @required BibleSearchDelegate<T> delegate,
  ...
  delegate
    ...
    ..translations = translations ?? delegate.translations;
  return Navigator.of(context).push(_SearchPageRoute<T>(
    delegate: delegate,
  ));
}

/// Delegate for [showBibleSearch] to define the content of the search page.
///
...
/// results of the search as returned by [BibleSearchDelegate.buildResults].
///
/// [BibleSearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [BibleSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [BibleSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [BibleSearchDelegate.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [showBibleSearch] about the selected search result.
///
/// A given [BibleSearchDelegate] can only be associated with one active [showBibleSearch]
/// call. Call [BibleSearchDelegate.close] before re-using the same delegate instance
/// for another [showBibleSearch] call.
abstract class BibleSearchDelegate<T> {
  ...
  Widget buildAutoCompleteSuggestions(BuildContext context) {
  ...
  /// communicate the result back to the initial caller of [showBibleSearch].
  ...
  /// to [showBibleSearch] that launched the search initially.
  ...
  /// Translations for word suggestions
  String _translations;
  String get translations => _translations;
  set translations(String value) {
    assert(value != null);
    _translations = value;
  }

  Future<AutoComplete> _autoComplete;
  Future<AutoComplete> get autoComplete => _autoComplete;
  set autoComplete(Future<AutoComplete> value) {
    assert(value != null);
    _autoComplete = value;
  }
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [BibleSearchDelegate.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [BibleSearchDelegate.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final BibleSearchDelegate<T> delegate;
...
class _SearchPageState<T> extends State<_SearchPage<T>> {
  ...
  void _onFocusChanged() {
    if (focusNode.hasFocus) {
      widget.delegate._queryTextController.selection = TextSelection(
          baseOffset: 0,
          extentOffset: widget.delegate._queryTextController.text.length);
      if (widget.delegate._currentBody != _SearchBody.suggestions) {
    ...
    switch (widget.delegate._currentBody) {
      case _SearchBody.suggestions:
        body = KeyedSubtree(
          ...
          child: widget.delegate.buildAutoCompleteSuggestions(context),
        );
        break;
